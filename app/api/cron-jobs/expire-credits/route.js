...
export async function GET(req) {
  try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.creditTransaction.create({
          data: {
            ...
            description: `${pkg.packageType} package expired. ${
              creditsToDeduct / 2
            } consultation${creditsToDeduct / 2 !== 1 ? "s" : ""} unused.`,
        ...
        await tx.notification.create({
          data: {
            ...
            message: `Your ${pkg.packageType} package has expired. ${
              creditsToDeduct / 2
            } unused consultation${
              creditsToDeduct / 2 !== 1 ? "s were" : " was"
            } forfeited. Purchase a new package to continue.`,
            isRead: false,
            actionUrl: "/pricing",
          },
        });
      ...
      if (!existingNotification) {
        await prisma.notification.create({
          data: {
            ...
            message: `Your ${
              pkg.packageType
            } package expires in 7 days! You have ${
              pkg.creditsRemaining / 2
            } consultation${
              pkg.creditsRemaining / 2 !== 1 ? "s" : ""
            } remaining. Book now before they expire.`,
            isRead: false,
            actionUrl: "/doctors",
          },
        });
      ...
      if (!existingNotification) {
        await prisma.notification.create({
          data: {
            ...
            message: `URGENT: Your ${
              pkg.packageType
            } package expires in 3 days! Don't lose your ${
              pkg.creditsRemaining / 2
            } remaining consultation${
              pkg.creditsRemaining / 2 !== 1 ? "s" : ""
            }. Book now!`,
            isRead: false,
            actionUrl: "/doctors",
    ...
    // 12-hour warnings
    const packagesExpiringSoon12 = await prisma.creditPackage.findMany({
      where: {
        status: PackageStatus.ACTIVE,
        expiresAt: { gte: now, lte: twelveHoursFromNow },
        creditsRemaining: { gt: 0 },
      },
      include: { user: true },
    });

    for (const pkg of packagesExpiringSoon12) {
      const existingNotification = await prisma.notification.findFirst({
        where: {
          userId: pkg.userId,
          type: NotificationType.CREDIT_EXPIRY,
          message: { contains: "12 hours" },
          createdAt: {
            gte: new Date(now.getTime() - 6 * 60 * 60 * 1000), // avoid duplicates within 6 hours
          },
        },
      });
      ...
      if (!existingNotification) {
        await prisma.notification.create({
          data: {
            ...
            type: NotificationType.CREDIT_EXPIRY,
            title: "Credits Expiring Soon",
            message: `Heads up — your ${
              pkg.packageType
            } package expires in 12 hours. You have ${
              pkg.creditsRemaining / 2
            } consultation${
              pkg.creditsRemaining / 2 !== 1 ? "s" : ""
            } remaining. Book now!`,
    ...
    return NextResponse.json({
      ...
      warnings12Hours: packagesExpiringSoon12.length,
