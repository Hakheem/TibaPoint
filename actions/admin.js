...
export async function verifyDoctor(doctorId, notes = '') {
  ...
  try {
    ...
    await db.adminLog.create({
      data: {
        adminId: (await db.user.findUnique({ where: { clerkUserId: (await auth()).userId } })).id,
  ...
  try {
    ...
    const doctor = await db.user.update({
      ...
      data: {
        ...
        suspendedBy: (await db.user.findUnique({ where: { clerkUserId: (await auth()).userId } })).id,
      }
    })
  ...
  try {
    const doctor = await db.user.update({
      ...
      data: {
        ...
        bannedAt: new Date(),
        banReason: reason,
        bannedBy: (await db.user.findUnique({ where: { clerkUserId: (await auth()).userId } })).id,
      }
    })
  ...
  try {
    const doctor = await db.user.update({
      ...
      data: {
        ...
        isAvailable: false,
        deletedAt: new Date(),
        deletedBy: (await db.user.findUnique({ where: { clerkUserId: (await auth()).userId } })).id,
      }
    })
...
export async function updateDoctorProfile(doctorId, data) {
  ...
  try {
    const validFields = [
      'speciality', 'experience', 'bio', 'consultationFee',
      'licenseNumber', 'credentialUrl', 'phone', 'city', 'country'
    ]
  ...
  try {
    ...
    await db.notification.create({
      data: {
        ...
        message: `Your account has been ${amount > 0 ? 'credited' : 'debited'} with ${Math.abs(amount)} credits. ${reason}`,
      },
    })
...
export async function cancelAppointment(appointmentId, reason = '') {
  ...
  try {
    ...
    // Refund credits to patient
    const refundedCredits = appointment.creditsCharged - appointment.creditsRefunded
      ...
      stats: {
        ...
        revenue: {
          monthly: {
            ...
            total: (monthlyRevenue._sum.platformEarnings || 0) + (monthlyRevenue._sum.doctorEarnings || 0),
          },
          yearly: {
            platform: yearlyRevenue._sum.platformEarnings || 0,
            doctor: yearlyRevenue._sum.doctorEarnings || 0,
            total: (yearlyRevenue._sum.platformEarnings || 0) + (yearlyRevenue._sum.doctorEarnings || 0),
          },
        },
      }
    }
