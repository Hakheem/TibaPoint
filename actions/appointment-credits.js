 ...
 */
export async function refundCreditsForAppointment(
  userId,
  appointmentId,
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const creditsToRefund = Math.floor(
      (CREDITS_PER_CONSULTATION * refundPercentage) / 100
    );
    ...
    await tx.notification.create({
      data: {
        ...
        message: `You've been refunded ${creditsToRefund / 2} consultation${
          creditsToRefund / 2 !== 1 ? "s" : ""
        } (${refundPercentage}% refund) for your cancelled appointment.`,
        isRead: false,
...
export async function checkCreditsAvailable(userId) {
  ...
  const consultationsAvailable = Math.floor(
    user.credits / CREDITS_PER_CONSULTATION
  );
